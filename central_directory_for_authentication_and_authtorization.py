...
class KeystoneLdapControl(Driver,
    ...
    def connect_to_server(self, inputs):
        ...
        assert not password is None or not private_key is None #control 5
        counter = counter + 10 #aggiunto
    ...
    def retrieve_keystone_configuration(self, inputs):
        assert self.keystone_config_file is not None #control 6
        counter = counter + 10 #aggiunto
    ...
    def check_ldap(self, ldap_config):
        ...
        mcp.parse()
        assert mcp.sections.get("identity", None) is not None #control 7
        counter = counter + 10 #aggiunto
        assert mcp.sections.get("ldap", None) is not None #control 8
        counter = counter + 10 #aggiunto
        if mcp.sections.get("identity").get("driver") == "ldap" and mcp.sections.get("ldap").get("url") == self.ldap_url:# control 9 - aggiunto
            counter = counter + 10 #aggiunto
