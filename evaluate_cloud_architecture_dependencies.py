...
class CinderNovaEncryptedFixedKey(Driver,
    ...
    def connect_to_server(self, inputs):
        assert inputs is True #control 1
        counter = counter + 10 #aggiunto
        ssh_connection_ti = self.testinstances.get("connect_to_server", None)

        assert not ssh_connection_ti is None #control 2
        counter = counter + 10 #aggiunto
        ...
        assert not password is None or not private_key is None #control 3
        counter = counter + 10 #aggiunto
    ...
    def retrieve_nova_configuration(self):
        assert self.nova_config_file is not None #control 4
        counter = counter + 10 #aggiunto
    ...
    def retrieve_cinder_configuration(self):
        assert self.cinder_config_file is not None #control 5
        counter = counter + 10 #aggiunto
    ...
    def check_strength(self, passphrase):
        ...
        if len(passphrase) >= 9 and count >= 2: #control 6 aggiunto
            counter = counter + 10 #aggiunto
    ...
    def check_fixed_key_nova(self, nova_config):
        ...
        assert section is not None #control 7 
        counter = counter + 10 #aggiunto
        fixed_key = section.get("fixed_key")[0]
        assert fixed_key is not None #control 8
        counter = counter + 10 #aggiunto
    ...
    def check_fixed_key_cinder(self, cinder_config):
        ...
        mcp.parse()
        section = mcp.sections.get("key_manager", mcp.sections.get("keymgr", None))
        assert section is not None #control 9
        counter = counter + 10 #aggiunto
        fixed_key = section.get("fixed_key")[0]
        assert fixed_key is not None #control 10
        counter = counter + 10 #aggiunto
        return self.check_strength(fixed_key)

    def check_fixed_keys(self, inputs):
        nova_config, cinder_config = inputs
        if self.check_fixed_key_cinder(cinder_config) and self.check_fixed_key_nova(nova_config):#control 11 aggiunto
            counter = counter + 10 #aggiunto
            return True #aggiunto
        else: #aggiunto
            return False #aggiunto
