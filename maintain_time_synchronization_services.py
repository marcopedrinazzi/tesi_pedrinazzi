...
class NtpControl(Driver,
    ...
    def preconditions(self, inputs):
        ...
        assert len(expected_servers_list) > 0 #control 1
    ...
    def connect_to_server(self, inputs):
        assert inputs is True #control 2
        counter = counter + 10 #aggiunto
        ssh_connection_ti = self.testinstances.get("connect_to_server", None)
        assert not ssh_connection_ti is None #control 3
         counter = counter + 10 #aggiunto
        ...
        assert not password is None or not private_key is None #control 4
        counter = counter + 10 #aggiunto
    ...
    def identify_init_system(self, inputs):
        assert inputs is True #control 5
        counter = counter + 10 #aggiunto
    ...
    def verify_init_system(self, init_system):
        assert init_system in ("sysvinit", "systemd", "upstart", ) #control 6
        counter = counter + 10 #aggiunto
        return init_system
    ...
    def check_chrony_enabled(self, init_system):
        ...
        return init_system #AGGIUNTO ho tenuto il formato del metodo sopra
    
    def assert_ntp_or_chrony(self, inputs):
        assert self.has_chrony ^ self.has_ntp #control 7
        counter = counter + 10 #aggiunto
        return True
